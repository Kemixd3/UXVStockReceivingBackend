...
ReceivedGoodsController.post("/received_goods_items", async (req, res) => {
  ...
  try {
    ...
    const query = `
      INSERT INTO received_goods_items (received_goods_id, Name, Quantity, SI_number, is_batch, createdBy) 
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    ...
    res.status(201).json({ message: 'Received goods item posted successfully' });
  } catch (error) {
...
ReceivedGoodsController.get("/received_goods_items/:received_goods_id/:si_number", async (req, res) => {
  try {
    ...
    const { received_goods_id, si_number } = req.params;
